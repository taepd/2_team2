...
<script src="https://code.jquery.com/jquery-3.2.1.min.js"></script>
	...
	$(function(){
		...
		function page(){
			...
			if(($('#cp').val())>1){
				tmp +='<a href="EmpList.emp?cp=${cpage-1}&ps='+pagesize+'">이전</a>';
			}
			//page 목록 나열하기
			for(var i=1;i<=pagecount; i++){
				if($('#cp').val()==i){
					...
					tmp +=('<a href="EmpList.emp?cp='+i+'&ps='+pagesize+'">['+i+']</a>');
				}
			}
			//다음 링크
			if(($('#cp').val())<pagecount){
				tmp += '<a href="EmpList.emp?cp=${cpage+1}&ps='+pagesize+'">다음</a>';
			};
			$('#zero_config_paginate').append(tmp);
		};
		
		//페이지 링크 비동기
		$('#zero_config_paginate a').click(function(e){
			e.preventDefault();
			let data = {ps : $('#paging option:selected').val(),
				    cp : $('#cp').val() 
		           };		
